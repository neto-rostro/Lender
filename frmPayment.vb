...
Public Class frmPayment
    ...
    Private Sub frmPayment_Activated(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Activated
        Debug.Print("frmPayment_Activated")
        If p_TranStatus = "10" Then
            Me.Text = "CarTrade Payment"
        End If
        If pnLoadx = 1 Then
    ...
    Private Sub initButton()
        ...
        Panel2.Enabled = Not lbShow
        Panel5.Enabled = Not lbShow
    ...
    Private Sub cmdButton_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        Select Case lnIndex
            ...
            Case 3 ' Cancel Update
                If MsgBox("Do you want to discard all changes?", MsgBoxStyle.Information + MsgBoxStyle.OkCancel, "CarTrade Entry") = MsgBoxResult.Ok Then
                    selRow = 0
                    pnRow = 0
                    p_oTrans.NewTransaction()
                    Call newRecord()
            ...
            Case 4 'Save confirmation
                If p_oTrans.SaveTransaction() Then
                    ...
                    'If MsgBox("Do you want to print the transaction", MsgBoxStyle.Information + MsgBoxStyle.OkCancel, "LR Payment Printing") = MsgBoxResult.Ok Then
                    '    If Not p_oTrans.PrintTrans Then
                    '        MsgBox("Unable to print LR Payment!", MsgBoxStyle.Information, "LR Payment Entry")
                    '    End If
                    'End If
                    If MsgBox("Do you want to PAY the transaction", MsgBoxStyle.Information + MsgBoxStyle.OkCancel, "CarTrade Confirmation") = MsgBoxResult.Ok Then
                        If Not p_oTrans.PayTransaction(False) Then
                            MsgBox("Unable to PAY the transaction!", MsgBoxStyle.Information, "CarTrade Entry")
                        End If
                        MsgBox("Transaction was PAID successfully!", MsgBoxStyle.Information, "CarTrade Entry")
                        selRow = 0
                        pnRow = 0
                        ...
                        initButton()
                    End If
                    Call p_oTrans.OpenTransaction(p_oTrans.Master("sTransNox"))
                    p_nEditMode = xeEditMode.MODE_UPDATE
                    initButton()
                Else
            ...
            Case 5 'Browse
                If p_oTrans.SearchPayment("", True) Then
                    selRow = 0
                    pnRow = 0
                    loadMaster()
                    loadDetail()
            ...
            Case 2 ' update
                If Trim(IFNull(p_oTrans.Detail(p_oTrans.ItemCount - 1, "sAcctnmbr"))) = "" Then
                    ...
                    Exit Sub
                End If
                If txtField09.Text = "CLOSED" Or txtField09.Text = "OPEN" Then
            ...
            Case 1 ' POST   
                If Trim(IFNull(p_oTrans.Detail(p_oTrans.ItemCount - 1, "sAcctNmbr"))) = "" Then
                    MessageBox.Show("Please select a record to pay", "No Data",
                                MessageBoxButtons.OK, MessageBoxIcon.Error)
                    Exit Sub
                End If
                If txtField09.Text = "CLOSED" Or txtField09.Text = "OPEN" Then
                    If MsgBox("Do you want to PAY the transaction", MsgBoxStyle.Information + MsgBoxStyle.OkCancel, "CarTrade Confirmation") = MsgBoxResult.Ok Then
                        If Not p_oTrans.PayTransaction(True) Then
                            MsgBox("Unable to PAY the transaction!", MsgBoxStyle.Information, "CarTrade Entry")
                        End If
                        selRow = 0
                        pnRow = 0
                    ...
                    End If
                Else
                    MessageBox.Show("Unable to Pay transaction" + Environment.NewLine + "Already " + txtField09.Text + "", "Error!",
                                       MessageBoxButtons.OK, MessageBoxIcon.Error)
                End If
    ...
    Private Sub loadDetail()
        ...
        p_oTrans.Master(5) = Format(total, xsDECIMAL)
    End Sub
